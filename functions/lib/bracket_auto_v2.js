...
exports.onBracketMatchDone = void 0;
...
function slotFor(cur) {
    return (cur.idx % 2) === 1 ? 'A' : 'B';
}
exports.onBracketMatchDone = (0, firestore_1.onDocumentWritten)('events/{eventId}/matches/{matchId}', async (event) => {
    const { eventId, matchId } = event.params;
    const after = event.data?.after?.data();
    if (!after || after.phase !== 'bracket')
        return;
    const meta = parseId(matchId);
    if (!meta)
        return;
    // 승자 확정일 때만
    if (after.status !== 'done' || !after.winner)
        return;
    ...
    const roundSnap = await db.collection(`events/${eventId}/matches`)
        ...
        .get();
    const allDone = roundSnap.docs.every(d => d.data().status === 'done');
    if (allDone) {
