...
exports.cloudTasksWorker = void 0;
exports.enqueueCloudTask = enqueueCloudTask;
exports.enableCloudTasksMode = enableCloudTasksMode;
...
// Cloud Tasks로 작업 큐잉
async function enqueueCloudTask(job) {
...
async function enqueueFirestoreQueue(job) {
    ...
    await db.collection(collectionName).add({
        ...
        nextAttemptAt: admin.firestore.Timestamp.fromMillis(Date.now() + (job.delaySeconds || 0) * 1000),
        payload: job.payload,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        // TTL 설정
        ttlAt: admin.firestore.Timestamp.fromMillis(Date.now() + 7 * 24 * 60 * 60 * 1000 // 7일
        )
    });
}
// Cloud Tasks 워커 함수
exports.cloudTasksWorker = functions
...
// Slack 업데이트 처리
async function processSlackUpdate(payload) {
...
// 승인 만료 처리
async function processApprovalExpiry(payload) {
...
// 메트릭 업데이트 처리
async function processMetricsUpdate(payload) {
