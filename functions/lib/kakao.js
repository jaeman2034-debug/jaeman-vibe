/**
 * 🎤 Kakao Mini 스킬 - 야고 브리핑 완성형
 * "야고 브리핑 틀어줘" → 최신 AI 음성 리포트 자동 재생
 */
...
export const kakaoBriefing = functions
    ...
    try {
        ...
        // 최신 리포트 조회 (ai_voice_reports 컬렉션 사용)
        const snap = await db
        ...
        const summary = doc.summary || doc.ttsSummary || "AI 음성 리포트";
        const reportDate = doc.reportDate || "오늘";
        const totalCount = ((_a = doc.stats) === null || _a === void 0 ? void 0 : _a.totalCount) || 0;
        const totalValue = ((_b = doc.stats) === null || _b === void 0 ? void 0 : _b.totalValue) || 0;
        // 오디오 URL이 있는 경우
        if (audioUrl) {
...
export const kakaoYesterdayBriefing = functions
    ...
    try {
            ...
            const response = {
                ...
                template: {
                    outputs: [
                        {
                            simpleText: {
                                text: "🗓️ 어제 야고 브리핑\n\n어제는 생성된 브리핑이 없어요. 오늘 브리핑을 들어보시겠어요? 🎧"
                            }
                        }
                    ]
                }
        ...
        const doc = snap.docs[0].data();
        const audioUrl = doc.audioUrl;
        const summary = doc.summary || doc.ttsSummary || "어제 AI 음성 리포트";
        if (audioUrl) {
            const response = {
                version: "2.0",
                template: {
            ...
            const response = {
                ...
                template: {
                    outputs: [
                        {
                            simpleText: {
                                text: "📊 일주일 야고 요약\n\n최근 일주일간 생성된 브리핑이 없어요. 매일 00:00에 자동으로 생성됩니다. 🎧"
                            }
                        }
                    ]
                }
