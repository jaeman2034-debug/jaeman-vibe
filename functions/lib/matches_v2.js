...
exports.recomputeStandings = exports.reportMatch = exports.generateSingleElim = exports.generateRoundRobin = void 0;
...
function roundRobinPairs(teamIds, repeat = 1) {
    const teams = [...teamIds];
    if (teams.length % 2 === 1)
        teams.push('__BYE__');
...
function seedSingleElim(teamIds) {
    const n = teamIds.length;
    const pow = 1 << Math.ceil(Math.log2(Math.max(2, n)));
    const seeds = [...teamIds];
    while (seeds.length < pow)
        seeds.push('__BYE__');
    // 간단 시딩(1-vs-last)
    const pairing = [];
...
exports.generateRoundRobin = (0, https_1.onCall)(async (request) => {
    const { eventId, repeat = 1 } = request.data;
    ...
    if (teamIds.length < 2)
        throw new https_1.HttpsError('failed-precondition', '팀이 2개 이상 필요합니다');
    // 기존 그룹 매치 삭제
...
exports.generateSingleElim = (0, https_1.onCall)(async (request) => {
...
exports.reportMatch = (0, https_1.onCall)(async (request) => {
    const { eventId, matchId, scoreA, scoreB } = request.data;
    // reportMatch 권한
    if (!request.auth?.uid)
        throw new https_1.HttpsError('unauthenticated', 'login');
    ...
    const isBye = (id) => id === '__BYE__';
    // 이벤트 규칙 로드
    const ev = (await db.doc(`events/${eventId}`).get()).data();
    const rules = ev?.rules || { mode: 'points', winPts: 3, drawPts: 1, lossPts: 0 };
    let winner = null;
    let ptsA = 0, ptsB = 0;
    if (Array.isArray(request.data.setsA) && Array.isArray(request.data.setsB)) {
        // 세트제: setsA=[25,18,15], setsB=[20,25,13] 처럼 길이 동일
        const setsA = request.data.setsA;
        const setsB = request.data.setsB;
        let wa = 0, wb = 0;
    ...
    await db.runTransaction(async (tx) => {
        tx.set(mref, {
            scoreA,
            scoreB,
            setsA: request.data.setsA || null,
            setsB: request.data.setsB || null,
            status: 'done',
            endAt: now(),
            winner: winner || null
        }, { merge: true });
        ...
        if (m.phase === 'group' && m.teamA !== '__BYE__' && m.teamB !== '__BYE__') {
            const upd = (tid, forP, agP, w, l, d, pts) => {
            ...
            if (Array.isArray(request.data.setsA)) {
                // 세트제: for/against는 세트 득점 총합 집계(선택), 승패/무만 반영
                const SA = request.data.setsA.reduce((x, y) => x + y, 0);
                const SB = request.data.setsB.reduce((x, y) => x + y, 0);
                if (winner === m.teamA) {
...
/** 전체 스탠딩 재계산(멱등) */
exports.recomputeStandings = (0, https_1.onCall)(async (request) => {
    const { eventId } = request.data;
    await assertStaff(eventId, request.auth?.uid);
    // 초기화
