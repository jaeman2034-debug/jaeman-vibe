...
exports.suggestTeams = functions.https.onCall(async (data, ctx) => {
    var _a;
    const { eventId, teamCount = 2 } = data;
...
exports.saveTeams = functions.https.onCall(async (data, ctx) => {
    const { eventId, teams } = data;
    if (!ctx.auth?.uid)
