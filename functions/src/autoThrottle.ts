...
export class AutoThrottleTuner {
  // 스로틀 메트릭 기록
  static async recordMetrics(metrics: Omit<ThrottleMetrics, 'timestamp'>): Promise<void> {
  ...
  // 채널별 스로틀 설정 조회
  static async getThrottleConfig(channel: string): Promise<ThrottleConfig | null> {
    const doc = await db.collection('throttle_config').doc(channel).get();
    
    if (!doc.exists) return null;
    
    return doc.data() as ThrottleConfig;
  }

  // 채널별 스로틀 설정 저장
  static async setThrottleConfig(config: Omit<ThrottleConfig, 'lastTuned'>): Promise<void> {
  ...
  // 자동 튜닝 실행
  static async tuneAllChannels(): Promise<{
    tuned: number;
    skipped: number;
    errors: string[];
  }> {
    ...
    for (const channelDoc of channelsSnap.docs) {
      try {
        const config = channelDoc.data() as ThrottleConfig;
  ...
  // 채널 튜닝 필요성 확인
  private static async shouldTuneChannel(channel: string): Promise<boolean> {
    ...
    if (lastTuned > oneHourAgo) return false;
    ...
    // 튜닝이 필요한 조건들
    const needsTuning = 
      avgMetrics.rateLimitHits > 5 || // 5회 이상 429 에러
  ...
  // 채널 튜닝 실행
  static async tuneChannel(channel: string): Promise<{
    oldCapacity: number;
    newCapacity: number;
    ...
    const newConfig: ThrottleConfig = {
      ...
      lastTuned: admin.firestore.FieldValue.serverTimestamp() as admin.firestore.Timestamp,
      tuningHistory: [
  ...
  // 최근 메트릭 조회
  private static async getRecentMetrics(channel: string, hours: number): Promise<ThrottleMetrics[]> {
    ...
    return snap.docs.map(doc => doc.data() as ThrottleMetrics);
  }

  // 평균 메트릭 계산
  private static calculateAverageMetrics(metrics: ThrottleMetrics[]): {
    rateLimitHits: number;
    avgLatency: number;
    queueLength: number;
    successRate: number;
  ...
  // 스로틀 튜닝 히스토리 조회
  static async getTuningHistory(channel: string): Promise<ThrottleConfig['tuningHistory']> {
    const config = await this.getThrottleConfig(channel);
    return config?.tuningHistory || [];
  }

  // 스로틀 성능 리포트
  static async getThrottleReport(channel: string, days: number = 7): Promise<{
    channel: string;
    period: string;
    currentConfig: {
      capacity: number;
  ...
  // 긴급 스로틀 조정 (수동)
  static async emergencyThrottleAdjustment(
    channel: string,
    capacity: number,
    refillPerSec: number,
    reason: string
