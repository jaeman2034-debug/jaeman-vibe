...
async function occupancy(eventId: string) {
  const ev = (await db.doc(`events/${eventId}`).get()).data() as any;
  const cap = Number(ev?.capacity || 0);
  if (!cap) return { cap: 0, pre: 0, pct: 0, ev };
...
async function announce(eventId: string, title: string, body: string, kind: '80' | '100') {
  // 공지 문서 생성
  ...
  // 푸시
  await sendToTopic(topic(eventId, 'announce'), { title, body }, { 
    type: 'auto.occupancy', 
    eventId, 
    level: kind 
  });
...
export const onAttendeeWriteAutoNotify = functions.firestore
...
export const setAutomation = functions.https.onCall(async (data, ctx) => {
  const { eventId, occupancy } = data as { eventId: string; occupancy: boolean };
