...
async function purgeDeletedUsersInternal() {
  ...
  try {
    ...
    await logSystemEvent('account_purge_completed', {
...
export const dailyCleanup = onSchedule({
  ...
  try {
    ...
    await logSystemEvent('daily_cleanup_completed', {
    ...
    await logAuditEvent('daily_cleanup_failed', undefined, {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
...
export const weeklyCleanup = onSchedule({
  ...
  try {
    ...
    await logSystemEvent('weekly_cleanup_completed', {
      timestamp: now.toDate().toISOString()
    });
    ...
    await logAuditEvent('weekly_cleanup_failed', undefined, {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
...
export const monthlyCleanup = onSchedule({
  ...
  try {
    ...
    await logSystemEvent('monthly_cleanup_completed', {
      timestamp: now.toDate().toISOString()
    });
    ...
    await logAuditEvent('monthly_cleanup_failed', undefined, {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
});
 ...
 * 만료된 세션 정리
 */
async function cleanupExpiredSessions(db: admin.firestore.Firestore, cutoffTime: admin.firestore.Timestamp) {
 ...
 * 오래된 로그 정리
 */
async function cleanupOldLogs(db: admin.firestore.Firestore, cutoffTime: admin.firestore.Timestamp) {
 ...
 * 오래된 알림 정리
 */
async function cleanupOldNotifications(db: admin.firestore.Firestore, cutoffTime: admin.firestore.Timestamp) {
 ...
 * 만료된 캐시 정리
 */
async function cleanupExpiredCache(db: admin.firestore.Firestore, cutoffTime: admin.firestore.Timestamp) {
 ...
 * 일일 통계 업데이트
 */
async function updateDailyStats(db: admin.firestore.Firestore) {
 ...
 * 주간 통계 생성
 */
async function generateWeeklyStats(db: admin.firestore.Firestore) {
 ...
 * 월간 통계 생성
 */
async function generateMonthlyStats(db: admin.firestore.Firestore) {
 ...
 * 사용자 활동 분석
 */
async function analyzeUserActivity(db: admin.firestore.Firestore) {
 ...
 * 오래된 감사 로그 아카이브
 */
async function archiveOldAuditLogs(db: admin.firestore.Firestore, cutoffTime: admin.firestore.Timestamp) {
 ...
 * 데이터베이스 최적화
 */
async function optimizeDatabase(db: admin.firestore.Firestore) {
...
async function performSystemHealthCheck() {
  ...
  try {
    ...
    await logSystemEvent('health_check_passed', {
    ...
    await logAuditEvent('health_check_failed', undefined, {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
}
