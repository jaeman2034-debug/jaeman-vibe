/**
 * ?렒 Kakao Mini ?ㅽ궗 - ?쇨퀬 釉뚮━?? * "?쇨퀬 釉뚮━????댁쨾" ??理쒖떊 AI ?뚯꽦 由ы룷???ъ깮
 */
...
export const kakaoMiniBriefing = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      console.log("?렒 Kakao Mini 釉뚮━???붿껌:", req.body);
        ...
        const response: KakaoResponse = {
          ...
          template: {
            outputs: [
              {
                simpleText: {
                  text: "?ㅻ뒛? ?ъ깮??AI 釉뚮━?묒씠 ?놁뼱?? 留ㅼ씪 00:00???먮룞?쇰줈 ?앹꽦?⑸땲?? ?댁씪 ?ㅼ떆 ?쒕룄?대낫?몄슂! ?렒"
                }
              }
            ]
          }
      ...
      const summary = latestReport.summary || latestReport.ttsSummary || "AI ?뚯꽦 由ы룷??;
      const reportDate = latestReport.reportDate || "?ㅻ뒛";
      const totalCount = latestReport.stats?.totalCount || 0;
      const totalValue = latestReport.stats?.totalValue || 0;
...
export const kakaoMiniYesterdayBriefing = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const response: KakaoResponse = {
          ...
          template: {
            outputs: [
              {
                simpleText: {
                  text: "?댁젣???앹꽦??釉뚮━?묒씠 ?놁뼱?? ?ㅻ뒛 釉뚮━?묒쓣 ?ㅼ뼱蹂댁떆寃좎뼱?? ?렒"
                }
              }
            ]
          }
      ...
      const summary = yesterdayReport.summary || yesterdayReport.ttsSummary || "?댁젣 AI ?뚯꽦 由ы룷??;
        ...
        const response: KakaoResponse = {
          ...
          template: {
            outputs: [
              {
                simpleText: {
                  text: "理쒓렐 ?쇱＜?쇨컙 ?앹꽦??釉뚮━?묒씠 ?놁뼱?? 留ㅼ씪 00:00???먮룞?쇰줈 ?앹꽦?⑸땲?? ?렒"
                }
              }
            ]
          }
