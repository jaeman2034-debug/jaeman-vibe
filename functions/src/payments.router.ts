...
// 게이트웨이 자동 선택
function selectGateway(currency: string, gateway?: string): 'toss' | 'stripe' {
  if (gateway && gateway !== 'auto') {
    return gateway as 'toss' | 'stripe';
  }
...
// 스마트 결제 체크아웃 생성
export const createSmartCheckout = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const confirmStripePayment = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
  }
