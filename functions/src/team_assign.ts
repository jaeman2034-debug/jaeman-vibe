...
export const suggestTeams = functions.https.onCall(async (data, ctx) => {
  const { eventId, teamCount = 2 } = data as { eventId: string; teamCount?: number };
...
export const saveTeams = functions.https.onCall(async (data, ctx) => {
  const { eventId, teams } = data as { 
    eventId: string; 
