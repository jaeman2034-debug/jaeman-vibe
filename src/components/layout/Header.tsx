...
const nav = [
  ...
  { to: "/market", label: "마켓", submenu: [
    { to: "/market", label: "?�체 ?�품" },
    { to: "/market/nearby", label: "??주�?" },
    { to: "/market/new", label: "?�품 ?�록" }
  ]},
  { to: "/meetups", label: "모임" },
  { to: "/jobs", label: "채용" },
];
...
export default function Header() {
  const { open } = useModal();
  const uid = getUid();
  const dev = uid && canAccessDev({ uid } as any); // ?�시 ?�??캐스??
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-zinc-950/60 border-b">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center gap-3">
        <Link to="/start" className="font-extrabold text-lg">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={() => open("voice:asr")} className="px-3 py-1.5 rounded-xl border inline-flex items-center gap-2 hover:bg-zinc-100 dark:hover:bg-zinc-800" aria-label="?�성 ?�식">
            <IconMic /> ?�성
          </button>
          <NotificationBell />
          <ThemeToggle />
