...
export async function httpPing(opts?: { silent?: boolean }) {
  const r = await fetch('/api/ping?t=' + Date.now(), { cache: 'no-store' });
  if (!opts?.silent) console.debug('[diag.httpPing]', r.ok);
  return r.ok; // true
}
...
export function installDiagAPI() {
  ...
  (window as any).diag = {
    async httpPing(opts?: { silent?: boolean }) {
    ...
    fb() {
      // FB 상태 확인 (로그 없이)
      if (!FB) return { error: 'FB not found on window' };
      return {
        keys: Object.keys(FB),
    ...
    async firestorePing() {
      const db = FB?.db;
      if (!db) return { error: 'FB.db not found' };
    ...
    async storagePing() {
      ...
      if (!storage) return { error: 'FB.storage not found' };
    ...
    async openFilePickerAndUpload(productId?: string) {
      const storage = FB?.storage;
      const db = FB?.db;
      if (!storage || !db) return { error: 'FB.storage or FB.db missing' };
      
      try {
        const [{ ref, uploadBytesResumable, getDownloadURL }, { doc, updateDoc, arrayUnion }] =
