...
/** 모임 AI 분석 (제목 기반) */
export async function analyzeGroup(title) {
...
/** 구직 AI 분석 (제목 기반) */
export async function analyzeJob(title) {
    const lowerTitle = title.toLowerCase();
...
/** 필드 잠금 상태 확인 (상품) */
export function getFieldLockStatus(currentValues, userModifiedFields) {
    return {
...
/** 필드 잠금 상태 확인 (모임) */
export function getGroupFieldLockStatus(currentValues, userModifiedFields) {
    return {
        title: userModifiedFields.has('title') || !!currentValues.title,
        category: userModifiedFields.has('category') || !!currentValues.category,
        desc: userModifiedFields.has('desc') || !!currentValues.desc,
...
/** 필드 잠금 상태 확인 (구직) */
export function getJobFieldLockStatus(currentValues, userModifiedFields) {
    return {
        title: userModifiedFields.has('title') || !!currentValues.title,
...
export function applyAISuggestions(currentValues, aiResult, fieldLocks) {
    const suggestions = {};
...
export function applyGroupAISuggestions(currentValues, aiResult, fieldLocks) {
    const suggestions = {};
    if (!fieldLocks.title && aiResult.title) {
        suggestions.title = aiResult.title;
    }
...
export function applyJobAISuggestions(currentValues, aiResult, fieldLocks) {
    const suggestions = {};
    if (!fieldLocks.title && aiResult.title) {
        suggestions.title = aiResult.title;
    }
