...
export default function MarketCreate_AI_VoiceTTS() {
  ...
  const speak = (text: string) => {
    ...
    utterance.onstart = () => {
      console.log("?”Š ?Œì„± ì¶œë ¥ ?œì‘:", text);
    };

    utterance.onend = () => {
      console.log("?”Š ?Œì„± ì¶œë ¥ ?„ë£Œ");
    };
  ...
  const startListening = () => {
    ...
    recognition.onstart = () => {
      setIsListening(true);
      console.log("?¤ ?Œì„± ?¸ì‹ ?œì‘");
      speak("?Œì„± ?¸ì‹???œì‘?©ë‹ˆ?? ?í’ˆ???€??ë§ì??´ì£¼?¸ìš”.");
    };
    ...
    recognition.onresult = (e: any) => {
      ...
      speak(`?Œì„± ?…ë ¥???¸ì‹?ˆìŠµ?ˆë‹¤. "${text}" ?´ë?ì§€ë¥??…ë¡œ?œí•´ì£¼ì„¸??`);
    };
  ...
  const generateAIData = async (imageUrl: string | null) => {
    setAiAnalyzing(true);
    try {
      speak("AIê°€ ?í’ˆ??ë¶„ì„ ì¤‘ì…?ˆë‹¤. ? ì‹œë§?ê¸°ë‹¤?¤ì£¼?¸ìš”.");
      ...
      if (aiDescription) {
        ...
        speak(`AI ë¶„ì„???„ë£Œ?˜ì—ˆ?µë‹ˆ?? ${aiCategory} ì¹´í…Œê³ ë¦¬ë¡?ë¶„ë¥˜?˜ì—ˆ?µë‹ˆ??`);
      } else {
        setDesc("AI ë¶„ì„ ?¤íŒ¨ ???˜ë™?¼ë¡œ ?…ë ¥?´ì£¼?¸ìš”.");
        speak("AI ë¶„ì„???¤íŒ¨?ˆìŠµ?ˆë‹¤. ?˜ë™?¼ë¡œ ?…ë ¥?´ì£¼?¸ìš”.");
      }
  ...
  const generateAIDataDirect = async (imageUrl: string | null) => {
    try {
      ...
      speak("AI ë¶„ì„ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.");
    }
  };
  ...
  // ?”¥ Firestore ?€??  const handleSubmit = async () => {
    if (!imageFile) {
      speak("?´ë?ì§€ë¥??…ë¡œ?œí•´ì£¼ì„¸??");
      return;
    }
    if (!desc) {
      speak("AI ë¶„ì„???„ë£Œ???Œê¹Œì§€ ê¸°ë‹¤?¤ì£¼?¸ìš”.");
      return;
    }
    if (!price) {
      speak("ê°€ê²©ì„ ?…ë ¥?´ì£¼?¸ìš”!");
    ...
    setLoading(true);
    try {
      speak("?í’ˆ???±ë¡ ì¤‘ì…?ˆë‹¤. ? ì‹œë§?ê¸°ë‹¤?¤ì£¼?¸ìš”.");
      ...
      await addDoc(collection(db, "market_items"), {
        ...
        imageUrl,
        imageUrls: [imageUrl],
        voiceText: voiceText, // ?Œì„± ?ìŠ¤?¸ë„ ?€??        sellerId: "ai-voice-tts-user",
        location: coords ? {
          latitude: coords.lat,
        ...
        ttsEnabled: true, // TTS ?±ë¡ ?œì‹œ
      });

      speak("?í’ˆ ?±ë¡???„ë£Œ?˜ì—ˆ?µë‹ˆ?? ë§ˆì¼“ ?˜ì´ì§€ë¡??´ë™?©ë‹ˆ??");
      ...
      console.error(err);
      speak("?±ë¡ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤. ?¤ì‹œ ?œë„?´ì£¼?¸ìš”.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-2">?¤ YAGO VIBE AI ?Œì„± ?´ì‹œ?¤í„´??/h1>
      ...
      <p className="text-gray-600 mb-4">
        ?Œì„±?¼ë¡œ ?í’ˆ???¤ëª…?˜ê³  ?´ë?ì§€ë¥??…ë¡œ?œí•˜ë©?AIê°€ ?ë™?¼ë¡œ ë¶„ì„?˜ê³  ?Œì„±?¼ë¡œ ?¼ë“œë°±ì„ ?œê³µ?©ë‹ˆ??
      </p>
