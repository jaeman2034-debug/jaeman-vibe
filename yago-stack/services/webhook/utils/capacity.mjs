...
export async function setCapacity(meetupId, capacity) {
  return setBucketCaps(meetupId, { default: capacity });
}

export async function getAvailableSeats(meetupId) {
  const counter = await cleanupHolds(meetupId);
  const used = (counter.counters.paid.default || 0) + (counter.counters.pending.default || 0);
  return Math.max(0, (counter.counters.capacity.default || Infinity) - used);
}